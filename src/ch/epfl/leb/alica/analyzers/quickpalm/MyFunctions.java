 ...
 * Molecular (PT)/Institut Pasteur (FR). Adapted under GPL by Marcel Stefko,
 * 2017.
 */
...
class MyFunctions {
    ...
    public MyFunctions(boolean live_view) {
        if (live_view) {
            this.live_view = new ImagePlus("QuickPALM live view.");
            this.live_view.show();
        } else {
            this.live_view = null;
        }
     ...
     * corresponding coordinate in Z by comparing against the loaded
     * Z-calibration table. Only used it the particle is disturbed by
     * astigmatism.
     *
     * @param wmh the width-minus-height of a particle
     * @return corresponding z-position value, will return 9999 if wmh is out of
     * limits
     */
    ...
    /**
     * Grabs a new image on an observed folder on the case of the analysis being
    ...
    int detectParticles(ImageProcessor ip, MyDialogs dg, int nframe) {
        ...
        for (int n = 0; n <= dg.maxpart; n++) {
            ...
            if (ip.get(maxs[1], maxs[2]) < snrthresh) {
                break;
            } else if (getParticle(ip, mask, maxs, dg, ptable, nframe)) {
                ok_nparticles++;
                Roi roi = new Roi(maxs[1]-1,maxs[2]-1,3,3);
            ...
            if (dg.smartsnr) {
                if (last_ok_nparticles != ok_nparticles) {
                    ...
                    smartcounter = 0;
                } else if (ok_nparticles > 1 && smartcounter > dg.maxpart * 0.1) {
                    break;
                } else {
                    smartcounter++;
                }
